...
void LSM9DS1::initGyro()
{
  ...
  Serial.println("finished gyro init");
...
void LSM9DS1::initAccel()
{
  ...
  Serial.println("finished accel init");
...
{  
  ...
  for (ii = 0; ii < 3; ii++)
  {
    gyro_bias[ii] = gyro_bias[ii] / samples;
    gbias[ii] = calcGyro(gyro_bias[ii]);
    accel_bias[ii] = accel_bias[ii] / samples;
    abias[ii] = calcAccel(accel_bias[ii]);
  }
  for(int i = 0; i < 3; i++)
    {
      Serial.println(gbias[i]);
    }
